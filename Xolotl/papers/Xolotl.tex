...
introducing vast amounts of cover traffic, which uses bandwidth
that might be better used to grow the user base and thus increase
...
A central goal for our network architecture is that messaging is {\bf
  asynchronous} and must work reliably even if sender and receiver are
never online at the same time.  As a result, we require that messages
...
the chance of both being broken is naturally lower and thus is is
desirable to produce a scheme that combines the strengths of both
classic and post-quantum methods.  However, there are technical
obstacles to deploying a post-quantum key exchange in a mix network,
starting with the simple problem that messages tend to be larger
and computations more expensive.

In this article, we propose Xolotl, a stateful ``ratchet'' based
solution, inspired by the Axolotl
ratchet~\cite{TextSecure}, that extends the Sphinx mix net packet
format~\cite{Sphinx}.  Xolotl provides limited post-quantum
protections and forward secrecy in exchange for a small sacrifice of
leaking some (mostly harmless) correlating information and increased
path length.  Xolotl also enables system architects the liberty to
make trade-offs between the amount of state routers have to keep and
the key rotation frequency, without this trade-off significantly
impacting forward secrecy.  As a result, the key rotation frequency
can be set to the maximum possible value limited only by expected
bandwidth utilization and available storage capacity.  This is
important as short key lifetimes can cause SURBs to expire, which
would cause messages or even sessions to be lost.
...
% FIXME: is this in the sphinx paper, or new?
% (Don't have the paper handy...)
% FIXME: needs a diagram, regardless ;-).
...
% NOTE: fixed the above, you had them wrong (pretty sure...)
...
% FIXME: I don't understand why. We use data from \epsilon
% to update the header, so why does this impact the sender's
% route at all?
...
% FIXME: I see that there is no reason for \alpha or \gamma to
% be in \epsilon, but I do not see how this impacts non-SURB
% packets, other than that we obviously like \epsilon to be
% short as it is overhead and all mixes must decrypt and process
% it, even if it is just garbage.  Is that your point? Then
% we might want to clarify it.
Instead we should make $\epsilon'$ shorter than the length $L$ of
$\beta$ by zero padding it at the cross over point.
% FIXME: I _suspect_ I understand, but most readers will
% not. Needs clarification and/or picture.
...
%like Elligator~\cite{elligator} to hide it from another hop.
 % NOTE: I do not understand the above; as all the other hops
 % will get the ephemeral curve point with onion encryption.
 % (The exception being the receiver, but the receiver knows
 % its cross over node and/or the path anyway.) So I do not
 % see why you want to go Elligator on me here...
...
% FIXME: 'hiccup -> issue'? 'fix -> address'?
...
% FIXME: why not? I think this is because we want to be able
% to repeatedly re-target a message (so we could have many
% SURB names, many unpredictable to the original sender); but
...
% FIXME: not sure, how would they be able to find a (valid/known)
% $\eta_0'$? They would have to know about a second message
% arriving for the very same client, so they would need to collude
% with additional peers on another path to the same client (or
% the client has to use the same path for more than one message).
...
% FIXME: I'd leave this last sentence out, as I do not
% think there is (1) a chance to actually use this to identify
% the source of a DOS, nor (2) a need in doing so.
