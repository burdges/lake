...
\begin{itemize}
\item Dining Cryptographers Neworks (DC-nets) \\
 \hspace*{2pt} Aims for even lower latency. $O(n^2)$ bandwidth. 
\item Private Inforation Retrieval (PIR) \\
...
Anonymity tools need ``onion encyption'' and some routing, but..

\bigskip

Onion Routing means progressively establishing a fixed channel, \\
\hspace*{2pt} negotating an ephemeral AKE with each hop. \\
\begin{itemize}
\item[Good] Stronger forward secrecy.  Inherently flexible.
\item[Bad] Easy corrolation attacks.
\end{itemize}
...
\begin{itemize}
\item[Good] Better against corrolation attacks.
\item[Bad] Relays cannot use ephemeral key material. 
\item[Mixed] Less flexible, facelitating analysis. \\
...
\begin{frame}{Archetecture}

Email handicapped past mix network efforst like Mixminion \\
...
\item the Single-Use Reply Block (SURB) is a second header, \\
 \hspace*{2pt} providing recipient and two-way anyonymity.
\end{itemize}
...
\bigskip
Are these measures sufficent?  No. \\
...
Sphinx is a packet format for an asychnornous mix network. 
